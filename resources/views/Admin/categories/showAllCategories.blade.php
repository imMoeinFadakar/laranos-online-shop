        ...
        <div action="/ChangeCategory" method="GET" class="row d-flex justify-content-center " >
            <div class="col-6 text-center" >
                <div class="alert alert-light" role="alert">
                    شماره     {{ $allCategory->id }}  :      {{ $allCategory->title }}
                </div>
            </div>
            <div class="col-4 d-flex justify-content-center align-items-center " >
                <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#exampleModal{{$allCategory->id}}">
           تغییر
                </button>
                <!-- Button trigger modal -->
                <button type="button" class="btn btn-danger mx-3" data-bs-toggle="modal" data-bs-target="#exampleModal{{$allCategory->id}}{{$allCategory->id}}">
                    حذف
                </button>

                <!-- Modal -->
                <div class="modal fade" id="exampleModal{{$allCategory->id}}{{$allCategory->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content orangeBGC text-white ">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5 m-auto" id="exampleModalLabel"> حذف کردن دسته بندی : {{ $allCategory->title }} </h1>
                            </div>
                            <div class="modal-body">
                                <div class="alert alert-light text-center" role="alert">
                                    <h4 class="text-center" >       پاک کردن دسته بندی :
                                        {{ $allCategory->title }}</h4>
                            ...
                            <div class="modal-footer">
                                ...
                                <a type="button" href="{{ url("/deleteCategory/$allCategory->id") }}" class="btn btn-danger">اره میخوام پاک بشه</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Modal -->
            <div class="modal fade" id="exampleModal{{$allCategory->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content orangeBGC text-white ">
                        <div class="modal-header text-center">
                            <h1 class="modal-title fs-5 m-auto" id="exampleModalLabel"> دسته بندی: {{ $allCategory->title }} </h1>
                        </div>
                        <div class="modal-body">

                            <form action="{{ url("/changeCategories") }}" method="POST" class="row d-flex justify-content-center " >
                                <input type="hidden" name="_token" value="{{csrf_token()}}" class="form-control" >
                                <input type="hidden"  name="categoryId" value="{{ $allCategory->id }}" class="form-control text-center" >
                                <div class="form-floating mb-3">
                                    <input type="text" value="{{ $allCategory->title }}" name="title" class="form-control" id="floatingInput" placeholder="name@example.com">
