...
<body>
    ...
    <div id="close" class=" w-100 position-absolute bgc z-3  " style="height:100vh" style="max-width: 25rem;top:30%; left:35%  " >
        ...
        <div class="card-header   orangeBGC text-white FontYekan text-center ">پیام مدیریت</div>
        <div class="card-body">
          <p class="card-text FontYekan text-center">{{ session("massage") }}</p>
          <h5 class="card-title"><button class="btn  btn__signup  FontYekan text-center"  id="closeHandler"  >تایید</button></h5>
        </div>
      </div>
    </div>
...
<div class="container d-flex justify-content-center m-auto">
 ...
 <h1 class="text-orange   FontYekan  " > ورود ادمین  </h1>
...
<form action="{{ url("/adminLoginPost")}}" method="POST" >
    ...
    <div class="form-floating mb-3 ">
        <input type="text" name="EUM"   
        @if (isset($_COOKIE["EUM"]))
        value="{{  $_COOKIE["EUM"]  }}"
                @endif 
        class="form-control   FontYekan text-orange  text-center" id="floatingInput" placeholder="name@example.com" required>
        <label for="floatingInput" class="  FontYekan text-center "> گذرواژه یا تلفن نام کاربری </label>
      </div>
      <div class="form-floating ">
        <input type="password" name="password" class="form-control text-center FontYekan text-orange "
